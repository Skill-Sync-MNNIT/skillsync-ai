...
export const AuthModal = ({ isOpen, onClose, pendingQuery }: AuthModalProps) => {
  ...
  return (
    ...
    >
      ...
      {/* Modal */}
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden animate-scale-in">
        {/* Close button */}
        <button
          onClick={onClose}
        ...
        <div className="px-8 py-10 text-center">
          {/* Icon */}
          <div className="mx-auto h-16 w-16 rounded-2xl bg-primary-50 flex items-center justify-center mb-6">
          ...
          {pendingQuery && (
            <div className="mt-5 px-4 py-3 bg-slate-50 rounded-xl border border-slate-100 text-left">
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Your search</p>
              <p className="text-sm text-slate-700 italic">"{pendingQuery}"</p>
            </div>
          )}
